  ...
  return {
    ...
    updateTradingSettings: (
      settings: Partial<
        Pick<
          AppSettings,
          'defaultSlippage' | 'autoSlippage' | 'gasPrice' | 'expertMode'
        >
      >
    ) => {
      updateSettings(settings);
...
export function useAISettings() {
  const { aiAssistant, smartSuggestions, riskWarnings, updateSettings } =
    useSettings();
  ...
  return {
    ...
    updateAISettings: (
      settings: Partial<
        Pick<AppSettings, 'aiAssistant' | 'smartSuggestions' | 'riskWarnings'>
      >
    ) => {
      updateSettings(settings);
    },
...
export function usePrivacySettings() {
  const { analytics, crashReporting, updateSettings } = useSettings();

  return {
    analytics,
    crashReporting,
    updatePrivacySettings: (
      settings: Partial<Pick<AppSettings, 'analytics' | 'crashReporting'>>
    ) => {
      updateSettings(settings);
    },
  };
...
export function validateSettings(settings: Partial<AppSettings>): string[] {
  ...
  if (settings.defaultSlippage !== undefined) {
    if (
      settings.defaultSlippage < APP_CONFIG.TRADING.MIN_SLIPPAGE ||
      settings.defaultSlippage > APP_CONFIG.TRADING.MAX_SLIPPAGE
    ) {
      errors.push(
        `Slippage must be between ${APP_CONFIG.TRADING.MIN_SLIPPAGE}% and ${APP_CONFIG.TRADING.MAX_SLIPPAGE}%`
      );
    }
  }
